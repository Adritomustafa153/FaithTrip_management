...
<!-- insert part is here -->
<div class="container">
    <h2>Agency Reissue Form</h2>
    <form action="reissue_agent_sell.php" method="POST">
        <input type="hidden" name="sale_id" value="<?= $row['SaleID'] ?>">
        
        <!-- Row 1: Agent Name, Search, and Select Agent -->
        <div class="form-row">
            <div class="form-group">
                <label for="companyDropdown">Select Agent:</label>
                <input type="text" name="agentname" value="<?= htmlspecialchars($row['PartyName']) ?>" readonly>
            </div>
            <div class="form-group">
...
$(document).ready(function() {
    // Initially disable and hide the system dropdown
    $('#system').prop('disabled', true).css('opacity', '0.7');
    
    // When source changes
    $('#source').change(function() {
        var selectedSource = $(this).val();
        
        // Check if source is IATA (or any variant of IATA)
        if (selectedSource.includes('IATA')) {
            $('#system').prop('disabled', false).css('opacity', '1');
        } else {
            $('#system').prop('disabled', true).css('opacity', '0.7').val('');
        }
    });
