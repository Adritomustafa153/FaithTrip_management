...
function getTodaysFlightsCount($conn) {
    $today = date('Y-m-d');
    $query = "SELECT COUNT(*) as count FROM sales WHERE (
                (FlightDate = ? AND FlightDate != '0000-00-00') 
                OR (ReturnDate = ? AND ReturnDate != '0000-00-00')
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      ...
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        ...
        <li class="nav-item dropdown">
          <a data-mdb-dropdown-init class="nav-link dropdown-toggle" href="#" id="navbarDropdownMenuLink" role="button" aria-expanded="false">invoice List</a>
          <ul class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink"> 
        ...
        <li class="nav-item dropdown">
          <a data-mdb-dropdown-init class="nav-link dropdown-toggle" href="#" id="navbarDropdownMenuLink" role="button" aria-expanded="false">Accounts</a>
          <ul class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
        ...
        <li class="nav-item dropdown">
          <a data-mdb-dropdown-init class="nav-link dropdown-toggle" href="#" id="navbarDropdownMenuLink" role="button" aria-expanded="false">Finance & Banking</a>
          <ul class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
        ...
        <li class="nav-item dropdown">
          <a data-mdb-dropdown-init class="nav-link dropdown-toggle" href="#" id="navbarDropdownMenuLink" role="button" aria-expanded="false">Human Resource</a>
          <ul class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
        ...
        <li class="nav-item dropdown">
          <a data-mdb-dropdown-init class="nav-link dropdown-toggle" href="#" id="navbarDropdownMenuLink" role="button" aria-expanded="false">Add</a>
          <ul class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
    ...
    <div class="d-flex align-items-center">
      <a class="text-reset me-3" href="invoice_cart2.php"><i class="fas fa-shopping-cart"></i></a>
      <div class="dropdown">
        <a class="text-reset me-3 dropdown-toggle hidden-arrow" href="#" id="notificationsDropdown" role="button" data-mdb-dropdown-init aria-expanded="false">
          <i class="fas fa-bell"></i>
          <?php if ($totalNotifications > 0): ?>
            <span class="badge rounded-pill badge-notification bg-danger"><?php echo $totalNotifications; ?></span>
          <?php endif; ?>
        </a>
        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="notificationsDropdown">
          <?php if ($notificationCount > 0): ?>
            <li><a class="dropdown-item" href="todays_flights.php"><i class="fas fa-plane me-2"></i><?php echo $notificationCount; ?> flight(s) today</a></li>
          <?php endif; ?>
          <?php if ($iataReminder['show_reminder']): ?>
            <li><a class="dropdown-item" href="iata_payments.php"><i class="fas fa-money-bill-wave me-2"></i>IATA Payment Due: <?php 
              $displayAmount = 0;
              if (isset($iataReminder['amount']) && $iataReminder['amount'] > 0) $displayAmount = $iataReminder['amount'];
              elseif (isset($iataReminder['first_period']) && $iataReminder['first_period'] > 0) $displayAmount = $iataReminder['first_period'];
              elseif (isset($iataReminder['second_period']) && $iataReminder['second_period'] > 0) $displayAmount = $iataReminder['second_period'];
              echo number_format($displayAmount, 2) . ' (' . ($iataReminder['period'] ?? 'Period') . ')'; 
            ?></a></li>
          <?php endif; ?>
          <?php if ($totalNotifications === 0): ?>
            <li><a class="dropdown-item" href="#"><i class="fas fa-check-circle me-2"></i>No notifications</a></li>
          <?php endif; ?>
        </ul>
      </div>
      <div class="dropdown">
...
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function() {
        ...
        dropdowns.forEach(dropdown => { new mdb.Dropdown(dropdown); });
    });
</script>
</body>
</html>